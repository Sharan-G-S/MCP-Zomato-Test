...
// Initialize the main app (hide landing, show dashboard)
async function initializeApp() {
    console.log('[INIT] Initializing app from landing page');
...
// Quick search from category cards
async function quickSearch(category) {
    ...
    // Wait a bit for the app to initialize, then send the search
    setTimeout(() => {
...
// =============================================================
async function init() {
    // Check if we should show landing page or go straight to app
    // Show landing page by default unless user has already used the app
    const hasUsedApp = localStorage.getItem('zomato_has_used_app');
    
    if (!hasUsedApp) {
        // First time user - show landing page
        console.log('[INIT] First time user - showing landing page');
        if (landingPage) landingPage.style.display = 'block';
        if (app) app.style.display = 'none';
        return;
    }
    
    // Returning user - initialize app directly
    console.log('[INIT] Returning user - initializing app');
    await initializeMainApp();
}

async function initializeMainApp() {
    // Mark that user has used the app
    localStorage.setItem('zomato_has_used_app', 'true');
    ...
    // Request browser geolocation if not already detected
    if (!userLocation) {
        requestUserLocation();
    }
...
async function sendMessage() {
    ...
    try {
        ...
        // Keep loading indicator visible until response is fully rendered
        ...
        if (err.name === 'AbortError') {
            addErrorMessage('Request timed out. The AI is taking too long to respond. This might be due to a slow connection or the request being too complex. Please try again or rephrase your message.');
        } else {
            addErrorMessage('Failed to send message. Check if the server is running. Error: ' + err.message);
        }
        console.error('Chat error:', err);
        
        // Then keep loading indicator visible for minimum time
        const elapsedTime = Date.now() - typingStartTime;
        const remainingTime = Math.max(0, MIN_LOADING_TIME - elapsedTime);
...
function addMessageToDOM(role, content) {
    ...
    const avatarLabel = role === 'user' ? 'You' : 'Z';
    let renderedContent = '';
    let actionBtnHtml = '';
...
function buildAddressCard(data, rawResult) {
    ...
    if (items.length > 0) {
        ...
        items.forEach((addr, i) => {
            ...
            let address = safeGet(addr, 'address', 'full_address', 'formatted_address', 'value') || JSON.stringify(addr).substring(0, 150);
            
            // Fix duplicate location names (e.g., "Vinaygapuram, Vinaygapuram")
...
// =============================================================
function initTheme() {
    const savedTheme = localStorage.getItem('zomato-theme') || 'dark';
...
function toggleTheme() {
    ...
    localStorage.setItem('zomato-theme', newTheme);
